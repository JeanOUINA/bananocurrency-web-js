...
const tools = {
	...
	verifyBlock: (publicKey: string, block: BlockData) => {
		...
		return signer.verify(publicKey, block.signature,
				preamble,
				bananoAddress.bananoAddressToHexString(block.account),
				block.previous,
				bananoAddress.bananoAddressToHexString(block.representative),
	...
	addressToPublicKey: (input: string): string => {
		const cleaned = input
			.replace('ban_', '')
		const publicKeyBytes = bananoAddress.decodeBananoBase32(cleaned)
