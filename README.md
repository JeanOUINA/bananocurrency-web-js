...
Toolkit for Banano cryptocurrency client side offline implementations allowing you to build web- and mobile applications using Banano without ever compromising the user's keys by sending them out of their own device.
...
* Generate new "legacy" Banano wallets with mnemonic phrases (Used in the Kalium wallet)
* Import HD wallets with a mnemonic phrase or a seed
* Import "legacy" wallets with the Banano/Nano mnemonic phrase or seed
...
```console
npm install bananocurrency-web
```
### In web

```html
<script src="https://unpkg.com/bananocurrency-web@1.3.4" type="text/javascript"></script>
...
The library is able to generate, import and derive accounts for HD wallets and "legacy" Banano/Nano wallets. A HD wallet seed length is 128 hexadecimal characters while a "legacy" Banano/Nano wallet seed is 64 characters long.
...
```
### Tools
#### Converting Banano units
Supported unit values are BAN, BANOSHI, RAW.
...
> The base project isn't from [JeanOUINA](https://github.com/jeanouina), its from [numsu](https://github.com/numsu), please donate to them !
> The informations below are from @numsu
